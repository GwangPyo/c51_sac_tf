...
class Projection(object):
    def __init__(self, sess, graph, n_spt, v_min, v_max, delta, batch_size):
        self.graph = graph
        self.sess = sess
        self.batch_size = batch_size
        self.n_spt = n_spt
        self.v_min, self.v_max, self.delta = v_min, v_max, delta
        with self.graph.as_default():
            self.reward_ph = tf.placeholder(tf.float32, [batch_size])
            self.dones_ph = tf.placeholder(tf.float32, [batch_size])
            self.Tz_ph = tf.placeholder(tf.float32, [batch_size, n_spt])
            self.value_distr_ph = tf.placeholder(tf.float32, [batch_size, n_spt])
            self.support = tf.constant(np.arange(v_min, v_max, delta), dtype=tf.float32)
    ...
    def _build(self):
        with self.graph.as_default():
            Tz = tf.clip_by_value(self.Tz_ph, self.v_min, self.v_max)
            tensor_list = []

            for i in range(self.batch_size):
                projected_target_z = tf.Variable(tf.zeros_like(self.value_distr_ph[i]))
                support_row = tf.clip_by_value(Tz[i], self.v_min, self.v_max - 1e-4)
                p = ((support_row - self.v_min) % self.delta) / self.delta
                idx = tf.cast(((support_row - self.v_min) // self.delta), dtype=tf.int32)

                projected_target_z = tf.scatter_add(projected_target_z, idx, self.value_distr_ph[i] * p)
                projected_target_z = tf.scatter_add(projected_target_z, idx + 1, self.value_distr_ph[i] * (1 - p))
                tensor_list.append(projected_target_z)
            self.projection_op = tf.identity(tf.stack(tensor_list, axis=0))

    def __call__(self, reward_batch, dones_batch, Tz_batch, value_distr_batch):
        feed_dict = {
            self.dones_ph: dones_batch,
            self.reward_ph: reward_batch,
            self.Tz_ph: Tz_batch,
            self.value_distr_ph: value_distr_batch
        }
        ret = self.sess.run(self.projection_op, feed_dict)
        # clear fcking tensorflow..
        feed_dict[self.value_distr_ph] = -value_distr_batch
        self.sess.run(self.projection_op, feed_dict)

        return ret


class C51SACCVaR(OffPolicyRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 seed=None, n_cpu_tf_sess=None, v_min=-400, v_max=400, n_spt=256, cvar_alpha=0.2, src_model=None):
        ...
        super(C51SACCVaR, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
        ...
        self.projection_op = None
        self.min_qf_pi = None
        self.train_critic_op = None
        self.behavior_cloning_op = None
        self.behavior_cloning_optimizer = None
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    behavior_cloning_loss = tf.reduce_mean(huber_loss(tf.abs(self.deterministic_action - self.actions_ph)))
                    self.behavior_cloning_optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate_ph)
                    ...
                    min_qf_pi_cdf = tf.math.cumsum(min_qf_pi_distr, axis=-1)
                    adjust_pdf = tf.where(
                        tf.less_equal(min_qf_pi_cdf, self.cvar_alpha),
                        min_qf_pi_distr,
                        tf.zeros_like(min_qf_pi_distr)
                    )
                    adjust_pdf = tf.math.divide_no_nan(adjust_pdf, tf.reduce_sum(adjust_pdf, axis=-1, keepdims=True))
                    self.min_qf_pi = adjust_pdf
                    ...
                    loss_qf1_distr = tf.clip_by_value(qf1_distr, 1e-20, 1.0)
                    loss_qf2_distr = tf.clip_by_value(qf2_distr, 1e-20, 1.0)
                    qf1_loss = -tf.reduce_mean(
                        tf.reduce_sum(tf.log(loss_qf1_distr) * tf.stop_gradient(self.projection_ph), axis=-1)
                    )
                    qf2_loss = -tf.reduce_mean(
                        tf.reduce_sum(tf.log(loss_qf2_distr) * tf.stop_gradient(self.projection_ph), axis=-1)
                    )
                    ...
                    # to clip policy loss
                    qf_pi = tf.reduce_mean(self.support * adjust_pdf, axis=-1, keepdims=True)
                    cvar = tf.reduce_mean(tf.reduce_sum(self.support * adjust_pdf, axis=-1, keepdims=True))
                    ...
                    # entropy_term = tf.stop_gradient(tf.reduce_mean(self.ent_coef * logp_pi))
                    value_fn_loss_distr = tf.clip_by_value(value_fn_distr, 1e-20, 1)
                    value_loss = -tf.reduce_mean(
                        tf.reduce_sum( tf.log(value_fn_loss_distr) * tf.stop_gradient(min_qf_pi_distr), axis=-1)
                    )
                    ...
                    with tf.control_dependencies([policy_train_op]):
                        ...
                        self.train_critic_op = train_values_op
                        self.behavior_cloning_op = [self.behavior_cloning_optimizer.minimize(behavior_cloning_loss),
                        ...
                        if ent_coef_loss is not None:
                            with tf.control_dependencies([train_values_op]):
                                ...
                                self.infos_names += ['ent_coef_loss', 'ent_coef', "cvar {}% quantile".format(np.round(100 * self.cvar_alpha))]
                                self.step_ops += [ent_coef_op, ent_coef_loss, self.ent_coef, cvar]
                    ...
                    tf.summary.scalar("cvar {}% quantile".format(np.round(100 * self.cvar_alpha)), cvar)
                    if ent_coef_loss is not None:
    ...
    def _train_step(self, step, writer, learning_rate):
        ...
        if self.log_ent_coef is not None:
            ent_coef_loss, ent_coef, cvar = values[-3:]
            return policy_loss, qf1_loss, qf2_loss, value_loss, entropy, ent_coef_loss, ent_coef, cvar
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if self.src_model is None:
                    ...
                    unscaled_action, _ = self.src_model.predict(obs)
    ...
    def _train_bc_step(self, learning_rate):
        batch = self.replay_buffer.sample(self.batch_size, env=self._vec_normalize_env)
        batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones = batch
        target_support, vf_target = self.sess.run([self.q_backup_op, self.value_target_distr], feed_dict={self.next_observations_ph: batch_next_obs,
                                                                      self.rewards_ph: batch_rewards.reshape(self.batch_size, -1),
        ...
        _, _, loss = self.sess.run(self.behavior_cloning_op, feed_dict)
        return loss

    def learn_behavior_clonning(self, total_timesteps,
              log_interval=400, tb_log_name="SAC", reset_num_timesteps=True, replay_wrapper=None):
            ...
            for step in range(total_timesteps):
                ...
                if self.num_timesteps % self.train_freq == 0:
                    ...
                    for grad_step in range(self.gradient_steps):
                        ...
                        mb_infos_vals.append(self._train_bc_step(current_lr))

                    if len(mb_infos_vals) > 0:
                        infos_values = [np.mean(mb_infos_vals)]
                ...
                if self.verbose >= 1 and done and log_interval is not None and num_episodes % log_interval == 0:
                    ...
                    if len(infos_values) > 0:
                        for (name, val) in zip(["loss"], infos_values):
                            logger.logkv(name, val)
                    logger.logkv("total timesteps", self.num_timesteps)
                    logger.dumpkvs()
                    # Reset infos:
...
if __name__ == '__main__':
    import gym
    from c51_sac.policy import LnMlpPolicy
    from stable_baselines import SAC
    env = gym.make("LunarLanderContinuous-v2")
    src = SAC.load("source_model.zip", env=env)

    model = C51SACCVaR(env=env, policy=LnMlpPolicy, verbose=1)
    model.learn(1000000)
    model.save("c51_sac_cvar_1M")
    # model.save("c51_sac_cvar_from_soruce")